...
# History page
    ...
    # Add these functions to the frontend_streamlit/app_frontend_timeseries_approach.py file

# Additional API functions for meal history
...
# Update the History page in the Streamlit app
if page == "History":
    st.header("Meal History")
            ...
            if st.session_state.meal_history_data:
                ...
                if not meals:
                    ...
                    if st.session_state.history_view == "detail" and st.session_state.selected_meal:
                        ...
                        if meal_detail and 'meal' in meal_detail:
                            ...
                            with col1:
                                ...
                                if 'image_path' in meal:
                                    ...
                                    # In a real app, you'd fetch the image from the server
                                    # Here we just show a placeholder
                                    st.image("https://via.placeholder.com/300x200?text=Meal+Image", 
                                             caption="Meal image", use_container_width=True)
                                
                                # Display meal timestamp
                                st.markdown(f"**Date & Time:** {format_timestamp(meal['timestamp'])}")
                                ...
                                for food in meal.get('food_items', []):
                                    ...
                                    with st.expander(f"Details for {food['food_name']}"):
                                        st.markdown(f"**Base GI:** {food.get('base_gi', 'N/A')}")
                                        
                                        if 'nutrients' in food:
                                            st.markdown("**Nutrients:**")
                                            for nutrient, value in food['nutrients'].items():
                                                st.markdown(f"- {nutrient.capitalize()}: {value}g")
                            
                            with col2:
                                # Display glucose prediction
                                ...
                                if 'feedback' in meal:
                                    ...
                                    st.success("Feedback submitted on " + format_timestamp(feedback['timestamp']))
                                    ...
                                    if 'user_response' in feedback and feedback['user_response']:
                                        ...
                                        st.markdown(f"**Glucose Impact:** {response_text}")
                                    ...
                                    if 'glucose_readings' in feedback and feedback['glucose_readings']:
                                        ...
                                        readings = feedback['glucose_readings']
                                        for reading in readings:
                                            timestamp = format_timestamp(reading['timestamp'])
                                            value = reading['value']
                                            st.markdown(f"- {timestamp}: {value} mg/dL")
